...
import { dashboardState, openState, recordAudit, redactPublic } from "./lib/state.mjs";
...
</style></head><body><div class="shell"><header class="top"><div class="brand">PICKBITS <b>DEPENDENCY AUDIT</b> <span class="status">ZERO TRUST</span></div><div class="status" id="updated">Loading evidence…</div></header>
...
// The server binds 127.0.0.1, but that alone does not stop a hostile web page
// from reaching it via DNS rebinding (resolving an attacker domain to 127.0.0.1
// so the victim's browser POSTs/GETs here). This dashboard exposes the full
// vulnerability inventory and canary markers, so reject any request whose Host
// header is not a loopback name.
...
const server = http.createServer((request, response) => {
  ...
  if (request.method === "GET" && url.pathname === "/api/state") {
    ...
    try {
      const state = dashboardState(db);
      sendJson(response, 200, publicView ? redactPublic(state) : state);
    } finally { db.close(); }
    return;
  }
