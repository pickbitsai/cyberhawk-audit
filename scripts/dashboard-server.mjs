...
const databasePath = path.resolve(args.db || "reports/cyberhawk-state.db");
const port = Number(args.port || 8787);

const html = String.raw`<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>CyberHawk Zero Trust</title><style>
...
const publicMode=new URLSearchParams(location.search).get('view')==='public';if(publicMode){el('detectionSection').hidden=true;el('publicEvidence').classList.add('show');document.title='CyberHawk Zero Trust - Public Results'}
...
server.listen(port, "127.0.0.1", () => {
  console.log(`CyberHawk persistent dashboard: http://127.0.0.1:${port}/`);
