...
A personal feed is an optional bridge between the public CyberHawk vulnerability list and the open-source local scanner. It is not a scanner by itself and it does not replace OSV.
...
- Ask the CyberHawk skill to compare that watchlist with packages actually present in a local project.
...
node scripts/import-watchlist.mjs `
  ...
  --output .cyberhawk\my-watchlist.txt
...
node scripts/generate-report.mjs `
  ...
  --watchlist .cyberhawk\my-watchlist.txt `
  --watchlist-label "My watch queue" `
  --output reports\cyberhawk-report.html
```
...
The service stores the group name, product keywords, and selected severity/date filters because they are required to render the feed. Product-interest analysis is limited to rows where the user explicitly enabled the optional trend checkbox. The feed service does not require or store an email, repository, source code, manifest, lockfile, or dependency inventory.
